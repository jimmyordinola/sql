...
BEGIN
        ...
        BEGIN
            ...
            PRINT 'Último histórico disponible: ' + CAST(@MaxAnio AS VARCHAR) + '-' + RIGHT('0' + CAST(@MaxMes AS VARCHAR), 2)

            -- NUEVA LÓGICA: Si es TIPO 2 y último día del mes, NO modificar FechaDesde
            IF @tipo = 2 AND @EsUltimoDiaMes = 1
            BEGIN
                -- Marcar que se requiere cálculo de saldo intermedio
                SET @RequiereCalculoSaldoIntermedio = 1

                -- NO modificar @FechaDesde - mantener la fecha original del usuario
                PRINT 'DECISIÓN ESPECIAL: Es TIPO 2 + último día del mes'
                PRINT '  Se MANTIENE FechaDesde original: ' + @FechaDesde
                PRINT '  Se calculará saldo inicial desde histórico hasta fecha solicitada'
                PRINT '  RequiereCalculoSaldoIntermedio = 1'
            END
            ELSE
            BEGIN
        ...
        BEGIN
            ...
            -- y el usuario pidió desde el último día de un mes)
            -- ================================================================
        ...
        BEGIN
            -- ================================================================
            -- FLUJO NORMAL (cuando SÍ existe histórico o no es último día del mes)
            -- ================================================================
